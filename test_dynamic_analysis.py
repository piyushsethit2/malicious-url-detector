#!/usr/bin/env python3
"""
Comprehensive test script for the improved malicious URL detection system
Tests dynamic content analysis, feedback system, and whitelist management
"""
...
# Test URLs with more diverse patterns
TEST_URLS = {
    "safe": [
...
def provide_feedback(url, was_detected_as_malicious, was_actually_malicious):
    """Provide feedback to improve the system"""
    try:
        response = requests.post(
            "http://localhost:8080/api/scan/feedback",
            params={
                "url": url,
                "wasDetectedAsMalicious": was_detected_as_malicious,
                "wasActuallyMalicious": was_actually_malicious
            },
            timeout=10
        )
...
def test_whitelist_management():
    ...
    try:
        response = requests.post(
            "http://localhost:8080/api/scan/whitelist/add",
            params={"domain": "test-domain.com"},
            timeout=10
        )
        print(f"Add to whitelist: {'✅' if response.status_code == 200 else '❌'} {response.text}")
    except Exception as e:
    ...
    try:
        response = requests.post(
            "http://localhost:8080/api/scan/whitelist/remove",
            params={"domain": "test-domain.com"},
            timeout=10
        )
        print(f"Remove from whitelist: {'✅' if response.status_code == 200 else '❌'} {response.text}")
    except Exception as e:
        print(f"❌ Error removing from whitelist: {e}")
    
    # Test getting domain reputations
    try:
        response = requests.get(
            "http://localhost:8080/api/scan/whitelist/domains",
            timeout=10
        )
        if response.status_code == 200:
            reputations = response.json()
            print(f"✅ Domain reputations retrieved: {len(reputations)} domains tracked")
        else:
            print(f"❌ Failed to get domain reputations: {response.status_code}")
    except Exception as e:
        print(f"❌ Error getting domain reputations: {e}")

def main():
    print("Testing Enhanced Malicious URL Detector")
    print("=" * 50)
    ...
    print("\nProviding Feedback for System Learning:")
    print("-" * 40)
    
    feedback_count = 0
    for result in all_results:
        if not result["correct"] and result["success"]:
            ...
            feedback_count += 1
            print(f"📝 Feedback provided for: {result['url']}")
    
    print(f"📊 Total feedback provided: {feedback_count}")
    
    # Save results to file
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"dynamic_test_results_{timestamp}.json"
    ...
    if safe_false_positives > 0:
        print(f"⚠️  {safe_false_positives} false positives detected. The system will learn from feedback.")
    else:
        print("✅ No false positives! Whitelist and threshold adjustments working well.")
    
    if malicious_false_negatives > 0:
        print(f"⚠️  {malicious_false_negatives} malicious URLs missed. The system will learn from feedback.")
    else:
        print("✅ All malicious URLs detected! Detection sensitivity is good.")
    
    if overall_accuracy >= 95:
        print("🎉 Excellent overall accuracy! The system is working very well.")
    elif overall_accuracy >= 90:
        print("👍 Very good overall accuracy. The system is learning and improving.")
    elif overall_accuracy >= 80:
        print("👍 Good overall accuracy. The system will improve with more feedback.")
    else:
        print("⚠️  Overall accuracy needs improvement. The system will learn from feedback.")
